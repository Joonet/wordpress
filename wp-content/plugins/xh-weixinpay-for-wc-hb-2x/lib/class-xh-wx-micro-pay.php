 ...
 *
 */
class XH_Wx_Micro_Pay
{
	/**
	 * 
	 * 提交刷卡支付，并且确认结果，接口比较慢
	 * @param WxPayXH_Wx_Micro_Pay $microPayInput
	...
	{
		//①、提交被扫支付
		$result = XH_Wx_Pay_Api::micropay($microPayInput, 5);
		//如果返回成功
		if(!array_key_exists("return_code", $result)
			|| !array_key_exists("out_trade_no", $result)
			|| !array_key_exists("result_code", $result))
		{
			throw new XH_Wx_Pay_Exception("接口调用失败！");
		}
	...
	{
		$queryOrderInput = new XH_WECHAT_WxPayOrderQuery();
		$queryOrderInput->SetOut_trade_no($out_trade_no);
		$result = XH_Wx_Pay_Api::orderQuery($queryOrderInput);
	...
	public function cancel($out_trade_no, $depth = 0)
	{
		...
		$clostOrder = new XH_WECHAT_WxPayReverse();
		$clostOrder->SetOut_trade_no($out_trade_no);
		$result = XH_Wx_Pay_Api::reverse($clostOrder);
